...
with mysql.connector.connect(host=host,user=user,password=password,db=db,port=port) as conn:
    ...
    cursor.execute("create table if not exists additems(itemid varchar(9) primary key,name varchar(30),category enum('fastfoods','vegfoods','nonvegfoods','pastry','icecreams','homefood','starters','soups'),price decimal(10,0),rid varchar(10),foreign key(rid) references admin(rid))")
